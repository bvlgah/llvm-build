...
from llvm_build.common.llvm import (
    LLVMProject,
    LLVMRuntime,
    LLVMSanitizer,
    LLVMTarget,
)
from llvm_build.common.utils import LoggerMixin
...
class LLVMBuildOptions(LoggerMixin):
    ...
    enableAssertions: bool | None = None
    enableProjects: Sequence[LLVMProject] = ()
    enableRuntimes: Sequence[LLVMRuntime] = ()
    buildTargets: Sequence[LLVMTarget] = ()
    useLLD: bool | None = None
    useCcache: bool | None = None
    useSanitizer: LLVMSanitizer | None = None
    buildExamples: bool | None = None
    cc: str | None = None
    cxx: str | None = None
    ...
    def parse(cls, args: Sequence[str]) -> "LLVMBuildOptions":
        ...
        options.enableProjects = [
            LLVMProject(proj) for proj in ns.enableProjects
        ]
        ...
        if ns.useSanitizer is not None:
    ...
    def _create_parser(cls) -> ArgumentParser:
        ...
        parser.add_argument(
            ...
            default=[],
            action="append",
            choices=[p for p in LLVMProject],
        ...
        parser.add_argument(
            ...
            default=[],
            action="append",
            choices=[r for r in LLVMRuntime],
        ...
        parser.add_argument(
            "--build-type",
            dest="buildType",
            choices=[t for t in BuildType],
            help="build type",
        )
        parser.add_argument(
            "--use-sanitizer",
            dest="useSanitizer",
            choices=[s for s in LLVMSanitizer],
            help="use a sanitizer",
        )
        parser.add_argument(
            "--src-dir",
            dest="srcDir",
        ...
        parser.add_argument(
            "--enable-assertions",
            dest="enableAssertions",
            action="store_true",
            help="Enable assertions in LLVM source code",
            default=None,
        )
        parser.add_argument(
            "--use-lld",
...
class LLVMCMakeBuilder(LoggerMixin):
    ...
    def _check_src_dir(self, srcDir: Path):
        ...
        if not srcDir.is_dir():
            raise RuntimeError(
                f"expect '{srcDir}' is the root directory of LLVM, "
    ...
    def _config_with_cmake(self, cmakePath: Path):
        ...
        if self._opts.enableAssertions is not None:
            args.append(
        ...
        if self._opts.enableProjects:
            projects = str.join(
                cmakeListSep, [p.value for p in self._opts.enableProjects]
            )
            args.append(f"-DLLVM_ENABLE_PROJECTS={projects}")
        if self._opts.enableRuntimes:
            runtimes = str.join(
                cmakeListSep, [r.value for r in self._opts.enableRuntimes]
            )
            args.append(f"-DLLVM_ENABLE_RUNTIMES={runtimes}")
        if self._opts.buildTargets:
            targets = str.join(
                cmakeListSep, [t.value for t in self._opts.buildTargets]
            )
            args.append(f"-DLLVM_TARGETS_TO_BUILD={targets}")
        if self._opts.useLLD is not None:
            args.append(
                f"-DLLVM_ENABLE_LLD={_to_cmake_bool(self._opts.useLLD)}"
            )
        if self._opts.useCcache is not None:
            args.append(
                f"-DLLVM_CCACHE_BUILD={_to_cmake_bool(self._opts.useCcache)}"
            )
        if self._opts.useSanitizer:
            args.append(f"-DLLVM_USE_SANITIZER={self._opts.useSanitizer.value}")
        if self._opts.buildExamples is not None:
            args.append(
                f"-DLLVM_BUILD_EXAMPLES={_to_cmake_bool(self._opts.buildExamples)}"
            )
        if self._opts.cc is not None:
            args.append(f"-DCMAKE_C_COMPILER={self._opts.cc}")
        if self._opts.cxx is not None:
            args.append(f"-DCMAKE_CXX_COMPILER={self._opts.cxx}")
        self._write_command_to_file(
            cmakePath, args, buildDir / "llvm_config.sh"
        )
        self._launch_binary_and_log(
    ...
    def _rename_file_with_timestamp(self, filePath: Path, compressed: bool):
        ...
        for newFilePath in generate_file_name():
            ...
            if not compressed:
                ...
                with (
                    filePath.open("rb") as uncompressedFile,
                    gzip.open(newFilePath, "wb") as compressedFile,
                ):
    ...
    def _build_with_ninja(self, ninjaPath: Path):
        ...
        self._launch_binary_and_log(
            ninjaPath, args, buildDir / "llvm_build.log"
        )
    ...
    def _launch_binary_and_log(
        ...
        with logFile.open("w") as outErrorFile:
            proc = subprocess.run(
                args=[binFile, *args], stdout=outErrorFile, stderr=outErrorFile
            )
